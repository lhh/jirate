...
def test_trans_string_reflective():
    ...
    assert transmogrify_input(fake_metadata, **inp)[0] == out
...
# Generated by Granite.code
def test_in_owc_valid():
    assert in_owc("option:child") == {'value': 'option', 'child': {'value': 'child'}}


def test_in_owc_invalid_length():
    with pytest.raises(ValueError):
        in_owc("option:child:extra")


def test_in_owc_no_colon():
    with pytest.raises(ValueError):
        in_owc("optionchild")
